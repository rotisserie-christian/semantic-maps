### Heuristics
- **`query_length`** - Age = tendency for longer vs shorter queries (younger = longer) <sup>1,6</sup>.
- **`complexity`** - Age + domain expertise = how elaborate the initial query is <sup>2,3,7</sup>.
- **`reformulation`** - Domain expertise + age = sticks to initial terms vs strategic reformulation / new keywords <sup>2,4,5</sup>.
- **`specificity`** - Age + prior task knowledge = vague vs specific phrasing <sup>2,6</sup>.
